...
def process_single_case_streaming(test_case: Dict[str, Any], doctor_config: Dict[str, Any], other_config: Dict[str, Any]):
    ...
    """
    doctor_system_message = f"""
    You are Dr. Agent, a medical doctor working to diagnose a patient based on the information provided by the Patient Agent and the Measurement Assistant. Your objective is to gather relevant history, perform necessary examinations, and request tests to reach a confident, specific diagnosis.

    You can only respond using **plain text dialogue** (no formatting, bullet points, or code blocks) and can perform **only one action per turn**:

    1. **Ask a focused history question** to the Patient Agent. Example: "Have you experienced any chest pain recently?"  
    Be clear and concise, asking questions that are directly relevant to the case.

    2. **Request a specific physical examination finding** from the Measurement Assistant using the format:  
    `REQUEST EXAMINATION FINDING: [specific examination]`  
    Example: `REQUEST EXAMINATION FINDING: assess the JVP`  
    Note that examination findings are free, and do not require cost details.

    3. **Request a specific test or investigation** from the Measurement Assistant using the format:  
    `REQUEST TESTS: [specific test]`  
    Example: `REQUEST TESTS: chest x-ray`  
    If the test result is unavailable, continue without it and adjust your approach accordingly.

    4. **Make your final diagnosis** once you believe you have sufficient information. Use the format:  
    `DIAGNOSIS READY: [specific diagnosis]`  
    Provide a clear, confident diagnosis and explain your reasoning based on the history, examination, and tests gathered so far.

        **Key Guidelines**:
    - Prioritize accuracy over speed. Take the time to gather all relevant information before making a final diagnosis.
    - You do not need to worry about the cost of tests or examinations. Focus entirely on getting the correct diagnosis.
    - Ensure your reasoning is clear and well-supported by the collected history, examination findings, and test results.
    - The Patient Agent responds based on the provided case, and the Measurement Assistant will provide results when requested.

    Your goal is to reach a clear, specific, and **correct** diagnosis through careful questioning, examination, and testing.
    """
    ...
    for i in range(max_turns):
        ...
        try:
            ...
            # --- TOKEN LOGGING START ---
            # Use tiktoken to count tokens for the input prompt
            model_name = None
            if hasattr(next_speaker, 'llm_config') and next_speaker.llm_config:
                config_list = next_speaker.llm_config.get('config_list', [])
            ...
            # Compose the input as a list of dicts (OpenAI format)
            input_token_count = 0
            all_input_tokens = []
            for m in formatted_messages:
                tokens = enc.encode(m['content'])
                input_token_count += len(tokens)
                all_input_tokens.extend(tokens)
            print(f"[TOKENS] {next_speaker.name} INPUT TOKENS: {input_token_count} (model: {model_name})")
            print(f"[TOKENS] {next_speaker.name} INPUT TOKEN IDS: {all_input_tokens}")
            print(f"[TOKENS] {next_speaker.name} INPUT DECODED: {enc.decode(all_input_tokens)}")
            # --- TOKEN LOGGING END ---
            ...
            reply_content = reply.get('content', '') if isinstance(reply, dict) else (reply if isinstance(reply, str) else '')
            output_tokens = enc.encode(reply_content)
            output_token_count = len(output_tokens)
            print(f"[TOKENS] {next_speaker.name} OUTPUT TOKENS: {output_token_count} (model: {model_name})")
            print(f"[TOKENS] {next_speaker.name} OUTPUT TOKEN IDS: {output_tokens}")
            print(f"[TOKENS] {next_speaker.name} OUTPUT DECODED: {enc.decode(output_tokens)}")
            # --- END TOKEN LOGGING OUTPUT ---
        except Exception as e:
